...
public class Tools {
     ...
     *             if the necessary size is larger than 2<sup>30</sup>.
     */
    public static int arraySize(final int expected) {
        final long s = Math.max(2, nextPowerOfTwo((long)expected));
        if (s > (1 << 30))
            throw new IllegalArgumentException("Too large (" + expected + " expected elements)");
        return (int) s;
    }
    // taken from FastUtil
