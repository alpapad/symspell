...
;
		int ii = 0;
		int jj = 0;
		while ((ii < leftArr.length) && (ii < rightArr.length)
				&& (leftArr[ii] == rightArr[ii])) {
			ii++;
		}

		while ((jj < (leftArr.length - ii)) && (jj < (rightArr.length - ii))
				&& (leftArr[leftArr.length - jj - 1] == rightArr[rightArr.length - jj - 1])) {
			jj++;
		}
